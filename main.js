...
app.post('/search', (req, response) => {
    ...
    axios.get('https://www.flipkart.com/search?q=' + req.body.search).then((res) => {
       ...
       // const maindiv = document.evaluate("//*[@id="(container)"]/div/div[3]/div/div[2]",document , null ,XPathResult.FIRST_ORDERED_NODE_TYPE,null,).singleNodeValue;
        //if (XPathResult)
        
       //const maindiv = xpath.evaluate("//div[@class ='_1YokD2 _3Mn1Gg']",document , null ,XPathResult.FIRST_ORDERED_NODE_TYPE,null,).singleNodeValue;
       
        //if (XPathResult)
        //var type = XPathResult.FIRST_ORDERED_NODE_TYPE;
        let maindiv = document.querySelectorAll('[class="_1YokD2 _3Mn1Gg"]')
        //let maindiv = document.querySelectorAll('[data-id]')
        console.log(maindiv.length)
        console.log(maindiv.textContent)
        let product = maindiv[0].querySelectorAll('[class="_1AtVbE col-12-12"]')
    ...
    axios.get('https://www.amazon.in/s?k=' + req.body.search).then((res) => {
        ...
        let am_productLink = [];
        for (let i = 2; i < 4; i++) {
            console.log(product[i].querySelector('i span'))
            console.log(product.length)
            am_productName.push(product[i].querySelector('h2 span').textContent);
            am_productPrice.push(product[i].querySelector('[class="a-offscreen"]').textContent);
            //i span
        ...
        console.log(amazon_data.length);
       // console.log(flipkart_data.length);
